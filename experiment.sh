...
> results_temp.txt
# echo "#Results,HTTP requests,Query execution time,CPU load,Memory usage" > results_temp.csv
...
probabilities=(0.0002 0.25 0.5)
iterations=1

todo=(5 6 7 10 11 12 14 15)

while read -u 3; do
  query=$REPLY
  ((query_counter++))
  suffix=""
  if [[ " ${todo[@]} " =~ " ${query_counter} " ]]; then
    ...
    for amount in "${person_amounts[@]}"; do
      ...
      for probability in "${probabilities[@]}"; do
        ...
        for ((i=1;i<=$iterations;i++)); do
          ...
          timeout -s 1 5m bash -c "node --trace-warnings experiment $amount$suffix $probability '$query' 2>&1 | tee -a results_temp.txt | tr -dc '0-9.\n' | tr '\n' ',' | sed 's/,,$//' >> results_temp.csv; echo >> results_temp.csv" &
          pid=$!
          wait $pid
