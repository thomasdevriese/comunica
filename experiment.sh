...
# > results3.txt
# echo "Query execution time,CPU load,Memory usage,#Results,HTTP requests" > results3.csv
...
while read -u 3; do
  ...
  for amount in "${person_amounts[@]}"; do
    ...
    for probability in "${probabilities[@]}"; do
      ...
      echo "Query $query_counter - $amount persons - probability $probability" >> results3.csv
      for ((i=1;i<=$iterations;i++)); do
        ...
        timeout -s 1 5m bash -c "node experiment $amount$suffix $probability '$query' 2>&1 | tee -a results3.txt | tr -dc '0-9.\n' | tr '\n' ',' | sed 's/,,$//' >> results3.csv; echo >> results3.csv" &
        pid=$!
        wait $pid
